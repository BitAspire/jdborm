...
import com.bitaspire.jdborm.schema.UuidVersion;
...
class SqlGenerationTest {
    ...
    // ── Declarative schema tests ───────────────────────────────────────────

    @Test
    void schemaToSqlCreatesTablesAndIndexes() {
        ...
        assertEquals(List.of(
                "CREATE TABLE IF NOT EXISTS users (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, email VARCHAR(255) NOT NULL UNIQUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email ON users (email)",
                "CREATE TABLE IF NOT EXISTS posts (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id INTEGER NOT NULL, title VARCHAR(200) NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id))",
                "CREATE INDEX IF NOT EXISTS idx_posts_user_id ON posts (user_id)"), schema.toSql());
    }

    @Test
    void schemaSupportsTypeSafeTableAndColumns() {
        Schema schema = Schema.create()
                .table(USERS, table -> table
                        .column(ID, "INTEGER", col -> col.primaryKey())
                        .column(NAME, "VARCHAR(100)", col -> col.notNull())
                        .index("idx_users_name", idx -> idx.on(NAME)));

        assertEquals(List.of(
                "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, name VARCHAR(100) NOT NULL)",
                "CREATE INDEX IF NOT EXISTS idx_users_name ON users (name)"), schema.toSql());
    }

    @Test
    void schemaSupportsUuidDefaults() {
        Schema schema = Schema.create()
                .table("api_keys", table -> table
                        .column("id", "UUID", col -> col.defaultPostgresUuid().primaryKey())
                        .column("mysql_id", "CHAR(36)", col -> col.defaultUuid(UuidDialect.MYSQL))
                        .column("hsqldb_id", "UUID", col -> col.defaultUuid(UuidDialect.HSQLDB)));

        assertEquals(List.of("CREATE TABLE IF NOT EXISTS api_keys (id UUID DEFAULT gen_random_uuid() PRIMARY KEY, mysql_id CHAR(36) DEFAULT UUID(), hsqldb_id UUID DEFAULT UUID())"),
                schema.toSql());
    }

    @Test
    void uuidsGenerateV4AndV7() {
        Instant instant = Instant.ofEpochMilli(1_717_171_717_171L);
        UUID v4 = Uuids.generate(UuidVersion.V4);
        UUID v7 = Uuids.timeOrderedV7(instant);

        assertEquals(4, v4.version());
        assertEquals(2, v4.variant());
        assertEquals(7, v7.version());
        assertEquals(2, v7.variant());
        assertEquals(1_717_171_717_171L, v7.getMostSignificantBits() >>> 16);
    }

    @Test
    void uuidsHaveShortAliases() {
        assertEquals(4, Uuids.v4().version());
        assertEquals(7, Uuids.v7().version());
    }
